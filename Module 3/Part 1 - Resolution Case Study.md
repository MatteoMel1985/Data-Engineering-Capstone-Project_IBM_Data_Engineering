...
From the ERD tool, click the button that generates or downloads the SQL script (the `Generate SQL` icon in the top ribbon).

As the task requires to create the schema in a database named `staging`, correct the SQL you will see on the monitor with the one below and run it.
